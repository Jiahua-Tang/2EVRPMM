...
rs_1 = [[1,2,1],[1,3,1]]#,[1,3,2,1],[1,2,3,1]]#,[1,4,2,1]]
...
# @variable(model, f[1:length(routes_1)]>=0, Int)
@variable(model, w[1:length(satellites)]>=0,Int)
...
model = Model(CPLEX.Optimizer)
set_silent(model)
@variable(model, x1[1:3, 1:3]>=0, Int)
@objective(model, Min, sum((arc_cost[i,j] - π_3[i]) * x1[i,j] for i in 1:3 for j in 1:3))

@constraint(model, flowDepot, sum(x1[1,j] for j in 1:3) ==1 )
@constraint(model, flowPoints[j in 1:3], sum(x1[i,j] for i in 1:3) == sum(x1[j,i] for i in 1:3))
# @constraint(model, sum(demands[i] * x[i,j] for i in 1:3 for j in 1:3) <= v_capacity)
# println(model)
optimize!(model)
println("reduced cost= ", objective_value(model))
