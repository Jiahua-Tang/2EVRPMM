...
function solveRestrictedLinearMasterProblem(branchDecisions::Union{Dict, Nothing} = nothing, branchKey::Union{Vector{Int}, Nothing} = nothing)
    ...
    for (r,route) in enumerate(routes_1)
        if value(x[r]) != 0
            println("   x=", value(x[r]), "1e Route ", route.sequence, "   Load= ", route[r].load ,  "   Cost= ", round(route.cost,digits=2))
        end 
    end
...
function solve_column_generation(rc1, routes_1,branchDecisions::Union{Dict, Nothing} = nothing, branchKey::Union{Vector{Int}, Nothing} = nothing)
    branchDecisions = isnothing(branchDecisions) ? Dict() : branchDecisions
    branchKey = isnothing(branchKey) ? Int[] : branchKey
...
maximum_duration_1e_vehicle = 100
...
global rc1, routes_1
rc1, routes_1, y = solve_column_generation(rc1, routes_1)
# println("")
println("reduced costs: $rc1")
println("1e routes: $routes_1")
