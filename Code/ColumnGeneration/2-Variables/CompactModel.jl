...
@variable(model,tau[C]>=0) #Cumulatedd distance
...
# @constraint(model, [i in P, j in P], t[i] + (1-x[i,j]) + distances[i,j]*x[i,j] <= t[j] + M*(1 - x[i,j]))
#23
#Time constraint for SEV and MTZ
# @constraint(model, [i in C, j in C], t[i]+ (1-z[i,j])+distances[i,j]*z[i,j] <= t[j]+M * (1 - z[i,j]))
#  #24
...
# @constraint(model, [p in P, j in C], t[p] + distances[p,j] * z[p,j] <= t[j])
...
println("Execution time: $(round(t1, digits=4)) seconds")