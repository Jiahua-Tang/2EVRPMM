...
function branchAndPricePer1eSubproblem(route_1e::Vector{Route}, routes_2e::Vector{Route})
    ...
    while !isempty(node_stack) && num_iter_sp < 31
        ...
        result = columnGenerationPer1eRoute(route_1e, routes_2e_pool, branchingInfo)    
        
        if !isnothing(result)
            y_value = result[2]
            routes_2e_pool = result[4]
            for route in result[5]
        ...
        num_iter_sp += 1
        println("Branching stack contains now $(length(node_stack)) nodes")
    end
end
