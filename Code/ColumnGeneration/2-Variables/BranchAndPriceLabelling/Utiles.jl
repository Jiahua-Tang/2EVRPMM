...
function transformRoutes(x)
    all_routes = []
    visited = Set{Tuple{Int, Int}}()

    for key in keys(x)
        (i, j) = key.value
        if round(value(x[i, j])) == 1 && (i, j) ∉ visited
            route = Vector{Int}()
            current_node = i
            ...
            while true
                found = false
                for next_key in keys(x)
                    (from, to) = next_key.value
                    if from == current_node && round(value(x[from, to])) == 1 && (from, to) ∉ visited
                        push!(visited, (from, to))
                        current_node = to
                        push!(route, current_node)
                        found = true
                        break
                ...
                if !found || current_node == route[1]  # end of route or cycle
                    break
                end
            end
...
function plotOriginal()
    ...
    scatter!(plt, x_vals[2:1+nb_parking][parking_availability[2:1+nb_parking] .== 1], 
                ...
                markershape=:utriangle, label="available", markersize = 6)

    scatter!(plt, x_vals[2:1+nb_parking][parking_availability[2:1+nb_parking] .!= 1], 
                y_vals[2:1+nb_parking][parking_availability[2:1+nb_parking] .!= 1], 
                markershape=:utriangle, color=:white, label="unavailable", markersize = 6)

    scatter!(plt, x_vals[1:1], y_vals[1:1], markershape=:square, label = "depot", markersize = 8)
