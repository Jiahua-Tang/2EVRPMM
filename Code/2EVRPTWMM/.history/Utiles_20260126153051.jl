...
function read_nico_dataset(filename)
    ...
    annotate!(plt, x_text, y_top-3, Plots.text("Number 2e vehicle per microhub: $(nb_vehicle_per_satellite)", :left, 8, color=:black))
    annotate!(plt, x_text, y_top-4.5, Plots.text("Microhubs: $minimum_parkings, Total parkings: $nb_parking", :left, 8, color=:black))
