...
function read_nico_dataset(filename)
    ...
    for i in customers
        annotate!(coor_x[i], coor_y[i]+0.7, text(node_labels[i-1-nb_parking], :center, 6)) 
        annotate!(plt, maximum(x for x in coor_x) + 5,
                       maximum(y for y in coor_y) - 15 - 1.5 * (i-nb_customer),
                       Plots.text(
                                  "cust[$i] : d=$(demands[i]), tw=[$(time_window_a[i]),$(time_window_b[i])]",
                                  :left,
                                  color=:black,
                                  8))
    end
