    ...
    while true # num_iter_cg < 2 # && true
        ...
        if has_values(model)
            ...
            execution_time_p = @elapsed begin
                routes_2e_pool, new_routes_from = pricing(selected_parkings, collect(1:length(routes_2e)), π1_stabilized, π2_stabilized, π3_stabilized, π4_stabilized, branchingInfo)
            end
