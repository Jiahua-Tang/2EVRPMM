...
function select_node_from_tree(node_stack)
    ...
    score =  0.5*node.fractionalScore/ (1+log(node.branchingInfo.depth))
    for node_iter in node_stack
        score_iter = (node_iter.cgLowerBound - base_lb + 0.5*node_iter.fractionalScore)/(1+log(node_iter.branchingInfo.depth))
        if score_iter < score
            node = node_iter
            score = score_iter
        end
        println("node $(node_iter.id) : LB = $(round(node_iter.cgLowerBound, digits=2)), baselb = $(round(base_lb, digits=2)), fractionalScore= $(round(node_iter.fractionalScore, digits=3)), rdt fs = $(round(node_iter.gradientFS, digits=3)), score = $(round(score_iter, digits=3))")
    end
