...
function solve_child_node(route_1e, node::BranchingNode, branching_decision::BranchingInfo, id)
    ...
    row_data = [Int(id), Int(node.parent_id),round(node.cgLowerBound,digits=3), round(node.fractionalScore, digits=3), round(node.gradientLB,digits=3), round(node.gradientFS, digits=3)]
    open("NodeMatrix.csv", "w") do file
