    ...
    while !isempty(label_queue) && num_new_columns < max_cols
        ...
        for node in active_nodes_set
            ...
            if !isnothing(new_label)
                ...
                #
                # NOTE: this used to also require new_label.reduced_cost < -1e-8
                # (only add the route if it's currently reduced-cost-improving).
                # That gate can hide routes that the master LP genuinely needs:
                # at a degenerate optimum, several other columns can sit at
                # reduced cost exactly 0 (tied, not pivoted in), and a column
                # that would only become improving *after* one of those ties is
                # resolved never gets the chance to enter the pool at all, since
                # pricing never revisits an already-non-improving candidate on
                # its own. Emitting every feasible closing move as a column
                # (subject to the existing branching/pool/dominance checks
                # below) lets the master LP itself resolve such ties directly,
                # instead of relying on reduced cost to gate which columns it's
                # even allowed to consider.
                if node in satellites_set && length(new_label.visitedSequence) > 2
