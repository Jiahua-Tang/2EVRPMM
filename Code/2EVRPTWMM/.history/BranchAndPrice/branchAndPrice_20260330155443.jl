        ...
        while !isempty(node_stack) # && num_iter_sp < 51
            ...
            if node.cgLowerBound > upperBound
                ...
                # execution_time = @elapsed begin
                    ...
                    #region: write node matrix
                    row_data = [node.id, node.parent_id,round(node.cgLowerBound,digits=3), round(node.fractionalScore, digits=3), round(node.gradientLB,digits=3), round(node.gradientFS, digits=3),current_node_id+1, current_node_id+2]
                    open("NodeMatrix.csv", "a") do file
                        println(file, join(row_data, ",")) 
                    end
                    #endregion
