...
function generate1eRoute(route)
    ...
    b2in = Vector{}()
    b2out = Vector{}()
    # 1e arrival times: cumulative arc cost along the FEV tour (no time windows on 1e nodes).
    # Depot (route[1]) starts at 0; each satellite is reached at previous_arrival + arc_cost.
    arrival_time = zeros(Float64, length(points))
    arrival_time[route[1]] = 0.0
...
#                 println("1e route $(best_route.sequence)  satellite arrival: ", [(n, round(best_route.arrival_time[n], digits=2)) for n in best_route.sequence if n in satellites])
#             end
#         end     
#     end    
...
function generate2eInitialRoutes()
    ...
    function generate_dummy_routes_by_chunks!()
        ...
        for startParking in satellites
            for endParking in satellites
                for i in 1:chunk_size:length(customers)
                    customer_group = customers[i:min(i + chunk_size - 1, length(customers))]

                    sequence = Vector{Int}()
                    push!(sequence, startParking)
                    append!(sequence, customer_group)
                    ...
                    push!(dummyRoutes_numeration, route.id)
                end
            end
        end
    end

    function generate_dummy_routes_simple!()
        for startParking in satellites
            for endParking in satellites
                sequence = Vector{Int}()
                push!(sequence, startParking)
    ...
    generate_dummy_routes_by_chunks!()
    # generate_dummy_routes_simple!()
        ...
        for parkingStart in satellites
            for parkingEnd in satellites
                if arc_cost[parkingStart, cust] + arc_cost[cust, parkingEnd] <= maximum_duration_2e_vehicle
                    if arc_cost[parkingStart,cust]<=time_window[cust][2]
                        route = generate2eRoute([parkingStart, cust, parkingEnd])
                        push!(routes_2e, route)
                        push!(routes_2e_by_start[parkingStart], route)                        
                    end
                end              
            end
        end
