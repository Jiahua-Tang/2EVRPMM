    ...
    if isnothing(node_stack)
        ...
        while !isempty(node_stack) # && num_iter_sp < 51
            ...
            if node.cgLowerBound > upperBound
                ...
                row_data = [round(node.id,digits=0), round(node.parent_id),round(node.cgLowerBound,digits=3), round(node.fractionalScore, digits=3), round(node.gradientLB,digits=3), round(node.gradientFS, digits=3),"prune"]
