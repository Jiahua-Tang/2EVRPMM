        ...
        while !isempty(node_stack) # && num_iter_sp < 51
            ...
            if node.cgLowerBound > upperBound
                ...
                # execution_time = @elapsed begin
                    ...
                    #region: write node matrix
                    appendNodeMatrix(node.y_value, node.id, node.parent_id, node.depth, node.cgLowerBound, node.fractionalScore, node.gradientLB, node.gradientFS,current_node_id+1, current_node_id+2)
                    #endregion
