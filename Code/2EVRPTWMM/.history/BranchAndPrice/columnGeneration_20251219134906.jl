...
function ng_labelling_optimized(π1, π2, π3, π4, selected_parkings, branchingInfo)
    ...
    while !isempty(label_queue) && num_new_columns < 50
        ...
        for node in active_nodes_set
            ...
            for forbidden_combination_customer_customer in branchingInfo.forbidden_served_together
                if forbidden_combination_customer_customer[1] in min_label.visitedSequence && forbidden_combination_customer_customer[2] in min_label.visitedSequence
                    continue
                end
            end
