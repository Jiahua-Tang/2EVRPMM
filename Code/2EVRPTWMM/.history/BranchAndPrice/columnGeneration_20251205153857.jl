...
function pricing(selected_parkings, routes_2e_pool::Vector{Int}, π1, π2, π3, π4, branchingInfo::BranchingInfo) 
    ...
    execution_time = @elapsed begin
        execution_time_sp = @elapsed begin
            subproblem_2e_result = ng_labelling_optimized(π1, π2, π3, π4, selected_parkings, branchingInfo)
        end
...
function ng_labelling_optimized(π1, π2, π3, π4, selected_parkings, branchingInfo)
    ...
    # Main labeling loop
    while !isempty(label_queue) && num_iter_labelling <= 500
        num_iter_labelling += 1
            ...
            if !isnothing(new_label)
                ...
                if node in satellites_set && new_label.reduced_cost < -1e-8 && length(new_label.visitedSequence) > 2
                    ...
                    if !is_dominated
                        ...
                        push!(depotLabels, new_label)
                        println(new_label.visitedSequence, "  ", round(new_label.reduced_cost, digits=2), "   +", round(generate2eRoute(new_label.visitedSequence).cost, digits=2), "   -", round(sum(π2[new_label.visitedSequence]), digits=2))#, "  ", round(calculateDualValueRoute(new_label.visitedSequence, π1, π2, π3, π4),digits=2))
                        push!(result, new_label)
                    end
    ...
    println("Completed: $num_iter_labelling iterations, $(length(result)) routes with negative reduced cost")
    
    # Convert back to standard Label format if needed for compatibility
    result_standard = [to_standard_label(l) for l in result]
