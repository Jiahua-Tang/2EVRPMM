...
function select_node_from_tree(node_stack)
    ...
    for node_iter in node_stack 
        score_iter = (node_iter.cgLowerBound - node.cgLowerBound + 0.5*node_iter.gradientFS)/(1+log(node_iter.branchingInfo.depth))
        if score_iter < score
            node = node_iter
            score = score_iter
        end
    ...
    if isnothing(node_stack)
        ...
        while !isempty(node_stack) # && num_iter_sp < 51
            ...
            if node.cgLowerBound > upperBound
                ...
                #region: write node matrix
                appendNodeMatrix(node.y_value, node.id, node.parent_id, node.cgLowerBound, node.fractionalScore, node.gradientLB, node.gradientFS, "Prune by LB","")
                #endregion
            else
