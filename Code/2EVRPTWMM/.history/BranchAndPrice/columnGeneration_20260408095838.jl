...
function pricing(num_iter_cg, selected_parkings, routes_2e_pool::Vector{Int}, π1, π2, π3, π4, π5, π6, branchingInfo::BranchingInfo) 
    ...
    execution_time_sp = @elapsed begin
        new_columns_found = ng_labelling_optimized(num_iter_cg, π1, π2, π3, π4, π5, π6, selected_parkings, branchingInfo)
    end
...
"""
function ng_labelling_optimized(num_iter_cg, π1, π2, π3, π4, π5, π6, selected_parkings, branchingInfo)
                ...
                if node in satellites_set && new_label.reduced_cost < -1e-8 && length(new_label.visitedSequence) > 2
                    ...
                    for route in routes_2e_by_start[new_label.visitedSequence[1]] 
                        if route.sequence == new_label.visitedSequence
                            # Existing route already in pool: reactivate its column if present
                            # println("Route already in pool: ", route.sequence, " id: ", route.id, " selection time: ", route.selectionTime)
                            # TODO clustering
                            # if haskey(y_vars, route.id)
                            #     y = y_vars[route.id]
                            #     route.selectionTime = num_iter_cg
                    ...
                    if !found_in_pool
                        ...
                        if !is_dominated
                            ...
                            push!(new_columns_found, new_route.id)
