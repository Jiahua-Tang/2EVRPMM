...
# end
#endregion
function pricing(selected_parkings, routes_2e_pool::Vector{Int}, π1, π2, π3, π4, π5, π6, branchingInfo::BranchingInfo, sat_arrival_times::Vector{Float64}; max_cols::Int = 100)



    execution_time_sp = @elapsed begin
        new_columns_found = ng_labelling_optimized(π1, π2, π3, π4, π5, π6, selected_parkings, branchingInfo, sat_arrival_times; max_cols = max_cols)
    end
...
function extendLabel_optimized(π2, π3, π4, label::LabelOptimized, next_node::Int, neighbours::Dict{Int, BitSet})
    ...
    if label.current_node == 3 && next_node == 6
        println("\ntest:: current node: $(label.current_node), next node: $next_node, M: $(label.M), neighbours: $(neighbours[next_node])")
    end
    if next_node in label.M
        return nothing
    end
    ...
    if label.current_node == 3 && next_node == 6
        println("test:: current node: $(label.current_node), $(label.earliest_time) next node: $next_node, M: $(label.M), neighbours: $(neighbours[next_node])")
    end
    # Update earliest feasible time
    ...
    if label.current_node == 3 && next_node == 6
        println("test:: current node: $(label.current_node), next node: $next_node, earliest feasible time: $(round(earliest_feasible_time, digits=2)), time window: $(time_window[next_node])")
    end
    if earliest_feasible_time > time_window[next_node][2]
        return nothing
    end
...
"""
function ng_labelling_optimized(π1, π2, π3, π4, π5, π6, selected_parkings, branchingInfo, sat_arrival_times::Vector{Float64}; max_cols::Int = 100)
    ...
    for parking in selected_parkings
        ...
        l = LabelOptimized(parking, rc, 0, 0, sat_arrival_times[parking], BitSet([parking]), [parking])
        ...
        for node in active_nodes_set
            ...
            if !isnothing(new_label)
                ...
                if node in satellites_set && new_label.reduced_cost < -1e-8 && length(new_label.visitedSequence) > 2
                    ...
                    # * branching rule : end-satellite – customer (MUST and FORBIDDEN)
                    #   - MUST (p, c): if route serves c, it must end at p
                    #   - FORBIDDEN (p, c): a route ending at p cannot serve c
                    #   Here `node` is the end satellite of the route.
                    #   (We allow routes that do not contain c at all.)

                    # MUST end-satellite – customer
                    for (p, c) in branchingInfo.must_include_end_combinations
                        if c in new_label.visitedSequence && node != p
                            branching_rule_legal = false
                            break
                        end
                    end
                    ...
                    # FORBIDDEN end-satellite – customer
                    for (p, c) in branchingInfo.forbidden_end_combinations
                        if node == p && c in new_label.visitedSequence
                            branching_rule_legal = false
                            break
                        end
                    end
