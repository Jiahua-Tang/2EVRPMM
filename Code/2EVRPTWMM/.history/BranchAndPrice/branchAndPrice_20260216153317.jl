...
function solve_column_generation(route_1e, branchingInfo::BranchingInfo, cgLB, fs, id, parent_id)
    ...
    execution_time_test = @elapsed begin
    while true # num_iter_cg < 2 # && true
        ...
        if has_values(model)
            ...
            println("--execution time duak: ", round(execution_time_dual, digits=3))
            # * 3. execute labelling algorithm
    ...
    println("--execution time loop: ", round(execution_time_test,digits=2))
