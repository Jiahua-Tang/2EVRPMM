...
function solve_column_generation(route_1e, filteredColumns::Vector{Int}, branchingInfo::BranchingInfo, cgLB, fs, id, parent_id)
    ...
    active_columns = deepcopy(filteredColumns)
    ...
    while true # num_iter_cg < 2 # && true
        ...
        if has_values(model)
            ...
            # display(new_columns_found)
            if length(new_columns_found) == 0
                break
            end

            active_columns = vcat(active_columns, new_columns_found)
        else
