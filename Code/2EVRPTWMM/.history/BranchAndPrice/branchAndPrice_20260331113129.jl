...
function appendNodeMatrix(y_value, status1, status2)
    active_routes = [
        join(routes_2e[idx].sequence, "-")
    ...
    row_data = [
        Int(round(node.id)),
        Int(round(node.parent_id)),
        round(node.cgLowerBound, digits=3),
        round(node.fractionalScore, digits=3),
        round(node.gradientLB, digits=3),
        round(node.gradientFS, digits=3),
        status1,
        status2,
        routes_str
    ]
...
function solve_column_generation(route_1e, branchingInfo::BranchingInfo, cgLB, fs, id, parent_id)
    ...
    if isLeaf
        ...
        appendNodeMatrix(y_values, "Integer","")
        active_routes = [
            join(routes_2e[idx].sequence, "-")
            for (idx, y) in enumerate(y_values) if y != 0
        ]

        routes_str = join(active_routes, " | ")

        row_data = [Int(id), Int(parent_id), round(total_obj,digits=3), round(fractionalScore, digits=3), round(total_obj - cgLB,digits=3), round(fractionalScore - fs, digits=3),"Integer","",routes_str]
        open("NodeMatrix.csv", "a") do file
            println(file, join(row_data, ",")) 
        end
        #endregion
    ...
    if isnothing(node_stack)
        ...
        while !isempty(node_stack) # && num_iter_sp < 51
            ...
            if node.cgLowerBound > upperBound
                ...
                #region: write node matrix
                appendNodeMatrix(node.y_value, "Prune","")
                #endregion
            else
                ...
                # execution_time = @elapsed begin
                    ...
                    active_routes = [
                        join(routes_2e[idx].sequence, "-")
                        for (idx, y) in enumerate(node.y_value) if y != 0
                    ]

                    routes_str = join(active_routes, " | ")
                    row_data = [round(node.id), 
                                round(node.parent_id), 
                                round(node.cgLowerBound,digits=3), 
                                round(node.fractionalScore, digits=3), 
                                round(node.gradientLB,digits=3), 
                                round(node.gradientFS, digits=3),
