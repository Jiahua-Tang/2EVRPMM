...
global branch_tree_positions = Dict{Int, Tuple{Int, Int}}()  # node_id => (row, col)
...
function solve_branch_and_price_2e_subproblem(route_1e::Route, node_stack)
    ...
    if isnothing(node_stack)
        ...
        # Log root node once when starting B&P on this subproblem, and initialize grid with header.
        try
            if !isempty(node_stack)
                root_node = node_stack[1]
                cg_str = string(round(root_node.cgLowerBound, digits = 2))

                # initialize grid with:
                # row 1: header (depth labels, starting at 0)
                # row 2: root node id (with \"ID \" prefix)
                # row 3: root node cg value (with \"CG result: \" prefix)
                open(branch_tree_file, "w") do io
                    println(io, "0")
                    println(io, "*******ID " * string(root_node.id) * "*******")
                    println(io, "CG result: " * cg_str)
                end

                # Append root node metrics as text lines below its cg row
                try
                    depth = root_node.branchingInfo.depth == 0 ? 1 : root_node.branchingInfo.depth
                    fs = root_node.fractionalScore
                    lb_red = root_node.gradientLB / depth
                    fs_red = root_node.gradientFS / depth
                    total_red = (-root_node.gradientLB + root_node.gradientFS) / (1 + log(depth))

                    lines = String[]
                    open(branch_tree_file, "r") do io
                        for ln in eachline(io)
                            push!(lines, chomp(ln))
                        end
                    end

                    # Expect current structure:
                    # row 1: header
                    # row 2: ID root
                    # row 3: cg root
                    grid = [split(ln, ",") for ln in lines]
                    ncols = maximum(length.(grid))
                    for row in grid
                    ...
                    # root value row is row index 3, column 1
                    col = 1
                    insert!(grid, 4, ["" for _ in 1:ncols])
                    grid[4][col] = "FS $(round(fs, digits=2))"
                    insert!(grid, 5, ["" for _ in 1:ncols])
                    grid[5][col] = "LB rdt $(round(lb_red, digits=2))"
                    insert!(grid, 6, ["" for _ in 1:ncols])
                    grid[6][col] = "FS rdt $(round(fs_red, digits=2))"
                    insert!(grid, 7, ["" for _ in 1:ncols])
                    grid[7][col] = "Total rdt: $(round(total_red, digits=2))"
                    ...
                    # ignore metrics logging errors
                end
            end
        catch
            # if branch_tree_file is not defined or not writable, skip logging
        end
        ...
        while !isempty(node_stack) # && num_iter_sp < 51
            ...
            if node.cgLowerBound > upperBound
                ...
                # Mark this node as pruned in branchTree.csv by appending " Pruned" after its ID.
                try
                    lines = String[]
                    open(branch_tree_file, "r") do io
                        for ln in eachline(io)
                ...
                # execution_time = @elapsed begin
                    ...
                    # Log created child nodes in branchTree.csv.
                    # Table structure:
                    #   - row 1: header (depth labels 0,1,2,...)
                    #   - subsequent rows come in PAIRS:
                    #       * id row   (node ids)
                    #       * value row (corresponding cg values)
                    # For a parent id at (row r, col c), place:
                    #   - first (left) child id in SAME id row r, column c+1; its cg in row r+1, column c+1
                    #   - second (right) child id in a NEW id row inserted after the parent's value row,
                    #     at column c+1; its cg in the value row just below.
                    try
                        if !isnothing(left_child_node) || !isnothing(right_child_node)
                            # Read existing grid
                            lines = String[]
                            open(branch_tree_file, "r") do io
                                for ln in eachline(io)
                            ...
                            # Find parent position by scanning for node.id on ID rows only
                            parent_id_str = "*******ID " * string(node.id) * "*******"
                            pr, pc = 1, 1
                            found_parent = false
                            for (ri, row) in enumerate(grid)
                                # skip header row (ri == 1) and value rows (odd index > 1)
                                if ri == 1 || isodd(ri)
                                    continue
                                end
                                for (ci, cell) in enumerate(row)
                                    if cell == parent_id_str
                                        pr, pc = ri, ci
                                        found_parent = true
                                        break
                                    end
                            ...
                            # Column for children is one to the right of parent
                            child_col = pc + 1

                            # Ensure all rows have at least child_col columns
                            for row in grid
                                while length(row) < child_col
                                    push!(row, "")
                                end
                            end

                            # Logical ids for children (even if pruned)
                            left_id  = current_node_id - 1
                            right_id = current_node_id

                            # 1) Place left child (or dummy) in the same ID row as parent
                            if !isnothing(left_child_node)
                                cg_str = string(round(left_child_node.cgLowerBound, digits = 2))
                                grid[pr][child_col] = "**ID " * string(left_child_node.id) * "**"
                                # ensure value row exists and has enough columns
                                while length(grid) < pr + 1
                                    push!(grid, ["" for _ in 1:ncols])
                                end
                                while length(grid[pr + 1]) < child_col
                                    push!(grid[pr + 1], "")
                                end
                                grid[pr + 1][child_col] = "CG result: " * cg_str
                            else
                                cg_str = string(round(node.cgLowerBound, digits = 2))
                                grid[pr][child_col] = "**ID " * string(left_id) * "**"
                                while length(grid) < pr + 1
                                    push!(grid, ["" for _ in 1:ncols])
                                end
                                while length(grid[pr + 1]) < child_col
                            ...
                            # 2) Place right child (or dummy) in a NEW ID row + VALUE row
                            # Insert two empty rows after parent's value row (pr+1):
                            #   - new_id_row at index pr+2
                            #   - new_val_row at index pr+3
                            insert!(grid, pr + 2, ["" for _ in 1:maximum(length.(grid))])
                            insert!(grid, pr + 3, ["" for _ in 1:maximum(length.(grid))])

                            # Ensure new rows have enough columns
                            while length(grid[pr + 2]) < child_col
                                push!(grid[pr + 2], "")
                            end
                            while length(grid[pr + 3]) < child_col
                                push!(grid[pr + 3], "")
                            end

                            if !isnothing(right_child_node)
                                cg_str = string(round(right_child_node.cgLowerBound, digits = 2))
                                grid[pr + 2][child_col] = "*******ID " * string(right_child_node.id) * "*******"
                                grid[pr + 3][child_col] = "CG result: " * cg_str
                            else
                                cg_str = string(round(node.cgLowerBound, digits = 2))
                                grid[pr + 2][child_col] = "*******ID " * string(right_id) * "*******"
                                grid[pr + 3][child_col] = "CG result: " * cg_str
                            end

                            # Update header row to reflect current depth columns: 0,1,2,...
                            header_cols = length(grid[1])
                            for c in 1:header_cols
                                grid[1][c] = string(c - 1)
                            end

                            # Append metrics text lines below cg rows for real child nodes
                            try
                                # Recompute ncols in case header grew
                                ncols2 = maximum(length.(grid))
                                for row in grid
                                    while length(row) < ncols2
                                        push!(row, "")
                                    end
                                end
                                ...
                                # Left child metrics
                                if !isnothing(left_child_node)
                                    depth_l = left_child_node.branchingInfo.depth == 0 ? 1 : left_child_node.branchingInfo.depth
                                    fs_l = left_child_node.fractionalScore
                                    lb_red_l = left_child_node.gradientLB / depth_l
                                    fs_red_l = left_child_node.gradientFS / depth_l
                                    total_red_l = (-left_child_node.gradientLB + left_child_node.gradientFS) / (1 + log(depth_l))

                                    # left cg is at (pr+1, child_col)
                                    base_row_l = pr + 1
                                    insert!(grid, base_row_l + 1, ["" for _ in 1:ncols2])
                                    grid[base_row_l + 1][child_col] = "FS $(round(fs_l, digits=2))"
                                    insert!(grid, base_row_l + 2, ["" for _ in 1:ncols2])
                                    grid[base_row_l + 2][child_col] = "LB rdt: $(round(lb_red_l, digits=2))"
                                    insert!(grid, base_row_l + 3, ["" for _ in 1:ncols2])
                                    grid[base_row_l + 3][child_col] = "FS rdt: $(round(fs_red_l, digits=2))"
                                    insert!(grid, base_row_l + 4, ["" for _ in 1:ncols2])
                                    grid[base_row_l + 4][child_col] = "Total rdt: $(round(total_red_l, digits=2))"
                                end

                                # Right child metrics
                                if !isnothing(right_child_node)
                                    # After inserting left metrics, right child's cg row has shifted down;
                                    # find it again by scanning for its ID in ID rows.
                                    right_id_str = "*******ID " * string(right_child_node.id) * "*******"
                                    rr_cg_row = 0
                                    rr_col = 0
                                    for (ri, row) in enumerate(grid)
                                        if ri == 1 || isodd(ri)
                                    ...
                                    if rr_cg_row != 0
                                        depth_r = right_child_node.branchingInfo.depth == 0 ? 1 : right_child_node.branchingInfo.depth
                                        fs_r = right_child_node.fractionalScore
                                        lb_red_r = right_child_node.gradientLB / depth_r
                                        fs_red_r = right_child_node.gradientFS / depth_r
                                        total_red_r = (-right_child_node.gradientLB + right_child_node.gradientFS) / (1 + log(depth_r))

                                        ncols3 = maximum(length.(grid))
                                        for row in grid
                                            while length(row) < ncols3
                                                push!(row, "")
                                            end
                                        end

                                        insert!(grid, rr_cg_row + 1, ["" for _ in 1:ncols3])
                                        grid[rr_cg_row + 1][rr_col] = "FS $(round(fs_r, digits=2))"
                                        insert!(grid, rr_cg_row + 2, ["" for _ in 1:ncols3])
                                        grid[rr_cg_row + 2][rr_col] = "Branching node with LB reduction: $(round(lb_red_r, digits=2))"
                                        insert!(grid, rr_cg_row + 3, ["" for _ in 1:ncols3])
                                        grid[rr_cg_row + 3][rr_col] = "Branching node with FS reduction: $(round(fs_red_r, digits=2))"
                                        insert!(grid, rr_cg_row + 4, ["" for _ in 1:ncols3])
                                        grid[rr_cg_row + 4][rr_col] = "Total reduction: $(round(total_red_r, digits=2))"
                                    end
                                end
                            catch
                                # ignore metrics insertion errors
                            end
                        ...
                        # logging failure is non-critical; ignore
                    end
