...
function extendLabel_optimized(π2, π3, π4, label::LabelOptimized, next_node::Int, neighbours::Dict{Int, BitSet})
    ...
    # if label.visitedSequence == [8,13,20] && next_node == 9
    #     println("current time = ", label.earliest_time)
    #     println("arc cost = ", round(arc_cost[label.current_node, next_node],digits=2))
    #     println("earliest_feasible_time = ",earliest_feasible_time)
    # end
    if earliest_feasible_time > time_window[next_node][2]
        return nothing
    end
