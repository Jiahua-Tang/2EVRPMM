...
function select_node_from_tree(node_stack)
    ...
    for node_iter in node_stack 
        score_iter = (node_iter.cgLowerBound - node.cgLowerBound + 0.5*node_iter.fractionalScore)/(1+log(node_iter.branchingInfo.depth))
        if score_iter < score
            node = node_iter
            score = score_iter
        end
        println(node_iter.id , "lb: $(round(node_iter.cgLowerBound,digits=2)), relative lb: $(round(node_iter.cgLowerBound - node.cgLowerBound,digits=3)) fs: $(round(node_iter.fractionalScore,digits=3)) score : ",score_iter)
    end
