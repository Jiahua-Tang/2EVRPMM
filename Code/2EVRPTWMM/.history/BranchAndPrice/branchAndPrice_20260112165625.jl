...
active_route_indices = Int[]

for k in sort(collect(keys(y_vars)))  # k is the route index
    y = y_vars[k]
    if JuMP.upper_bound(y) == 1.0
        push!(active_route_indices, k)
        # Example: do something with the column/route
        # println("active column: ", routes_2e[k].sequence)
    end
end
            # * 3. execute labelling algorithm
