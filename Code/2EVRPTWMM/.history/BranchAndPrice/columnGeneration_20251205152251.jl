            ...
            if !isnothing(new_label)
                ...
                if node in satellites_set && new_label.reduced_cost < -1e-8 && length(new_label.visitedSequence) > 2
                    ...
                    if !is_dominated
                        ...
                        push!(depotLabels, new_label)
                        println(new_label.visitedSequence, "  ", round(new_label.reduced_cost, digits=2)), "   ", generate2eRoute(new_label.visitedSequence).cost#, "  ", round(calculateDualValueRoute(new_label.visitedSequence, π1, π2, π3, π4),digits=2))
                        push!(result, new_label)
                    end
