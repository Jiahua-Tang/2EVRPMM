...
function calculateDualValueRoute(route::Vector{Int}, π1, π2, π3, π4, π5, π6)
    ...
    reduced_cost += π4[route[1]] * load - π5 + π6
                ...
                if node in satellites_set && new_label.reduced_cost < -1e-8 && length(new_label.visitedSequence) > 2
                    ...
                    for route in routes_2e_by_start[new_label.visitedSequence[1]] 
                        if route.sequence == new_label.visitedSequence
                            found_in_pool = true
                            for (route, k) in enumerate(routes_2e)
                                if route.sequence == route.sequence
                                    y = y_vars[k]
                                    break
                                end
