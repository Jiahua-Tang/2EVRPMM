...
function solve_child_node(route_1e, node::BranchingNode, branching_decision::BranchingInfo, id)
    ...
    execution_time = @elapsed begin
        columns_to_be_kept, columns_to_be_deleted = filter_2e_routes(branching_decision, node.active_routes)
    end
    global execution_time_filtering += execution_time
    ...
    execution_time = @elapsed begin
        ...
        for route_idx in 1:length(routes_2e)
            ...
            if haskey(y_vars, route_idx) && (route_idx in routes_2e_to_keep)
                y = y_vars[route_idx]
                JuMP.set_upper_bound(y, 1.0)
                JuMP.set_lower_bound(y, 0.0)
            end
    ...
    execution_time = @elapsed begin
        child_node = solve_column_generation(route_1e, columns_to_be_kept, node.cgLowerBound, node.fractionalScore, id, node.id) 
