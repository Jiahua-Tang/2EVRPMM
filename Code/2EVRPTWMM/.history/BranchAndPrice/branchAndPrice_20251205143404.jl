...
function solve_MILP_model_root(route_1e)
    ...
    @constraint(milpModel, [p in satellites], sum(y[idx]*r.b2out[p] for (idx, r) in enumerate(routes_2e_pool)) <=
                                              nb_vehicle_per_satellite* sum(x[idx]*r.b1[p] for (idx, r) in enumerate(routes_1e_complete)))
    @constraint(milpModel, [i in customers], sum(y[idx]*r.a[i] for (idx,r) in enumerate(routes_2e_pool)) >=1)
    @constraint(milpModel, [p in satellites], sum(y[idx]*r.b2in[p] for (idx,r) in enumerate(routes_2e_pool))== sum(y[idx]*r.b2out[p] for (idx,r) in enumerate(routes_2e_pool)))
    @constraint(milpModel, [p in satellites], sum(y[idx]*r.load*r.b2out[p] for (idx,r) in enumerate(routes_2e_pool))<= capacity_microhub)
