...
function solve_branch_and_price_2e_subproblem(route_1e::Route, node_stack)
    ...
    if isnothing(node_stack)
        ...
        row_data = [round(id), round(node.parent_id),round(node.cgLowerBound,digits=3), round(node.fractionalScore, digits=3), round(node.gradientLB,digits=3), round(node.gradientFS, digits=3)]
