...
function appendNodeMatrix(y_value, id, parent_id, depth, cgLowerBound, fractionalScore, gradientLB,gradientFS,status1, status2)
    active_routes = [
...
function solve_column_generation(route_1e, branchingInfo::BranchingInfo, cgLB, fs, id, parent_id)
    ...
    if total_obj > upperBound
        ...
        appendNodeMatrix(y_values, id, parent_id, 0, total_obj, 0, total_obj-cgLB, 0, "Prune by CG","")        
        #endregion
    ...
    if isLeaf
        ...
        #region: write node matrix
        appendNodeMatrix(y_values, id, parent_id, 0,total_obj, fractionalScore, total_obj-cgLB, fractionalScore-fs, "Integer","")
        #endregion
    ...
    if isnothing(node_stack)
        ...
        while !isempty(node_stack) # && num_iter_sp < 51
            ...
            if node.cgLowerBound > upperBound
                ...
                #region: write node matrix
                appendNodeMatrix(node.y_value, node.id, node.parent_id, node.depth, node.cgLowerBound, node.fractionalScore, node.gradientLB, node.gradientFS, "Prune","")
                #endregion
            else
                ...
                # execution_time = @elapsed begin
                    ...
                    #region: write node matrix
                    appendNodeMatrix(node.y_value, node.id, node.parent_id, branching_decisions[1].depth, node.cgLowerBound, node.fractionalScore, node.gradientLB, node.gradientFS,current_node_id+1, current_node_id+2)
                    #endregion
