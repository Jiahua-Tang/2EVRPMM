...
function read_nico_dataset(filename)
    ...
    # OPTION A:
    # we take only the first depot for the moment
    # we take all the satellites 
    # we generate some other parkings places according to the distance 
    # between the satellites points and the center of this plan

    # OPTION B:
    # with given number of parking places
    # set ceil(nb_parking/2) parkings as activated parkings
    # so we can calculate the lower bound of mm = total demand/nb mm
    # the upper bound of mm capacity = total demand
    # number of 2ev per mm = ceil(upper bound + lower bound)/2

    
    #==========================================================================
        SECTION 1: INITIALIZE PARAMETERS
    ==========================================================================#
    CAPACITY_1E = 2000                # First-echelon vehicle capacity
    ...
    ==========================================================================#
    lines = reverse(readlines(filename))
    ...
    for (idx, line) in enumerate(lines)
        ...
        if !isempty(line)
            parts = split(line)
            push!(coor_x, parse(Int, parts[1]))
            push!(coor_y, parse(Int, parts[2]))

            if length(parts) == 6
                nb_customer += 1
                push!(time_window_a, parse(Int, parts[3]))
                push!(time_window_b, parse(Int, parts[4]))
                push!(demands_data, parse(Int, parts[5]))
            else
                nb_parking += 1
                push!(time_window_a, 0)
                push!(time_window_b, 2000)
                push!(demands_data, 0)
            end
        end
    end
    ...
    # Calculate arc costs (distances)
    coor_full = [[x, y] for (x, y) in zip(coor_x, coor_y)]
    time_window_full = [[a, b] for (a, b) in zip(time_window_a, time_window_b)]
    arc_cost_matrix = calculate_arc_cost(coor_full)
    ...
    nb_microhub = Int(ceil(nb_parking/2))
    capacity_mm_ub = total_demand
    capacity_mm_lb = Int(ceil(total_demand/nb_microhub))
    capacity_microhub = Int(ceil((capacity_mm_lb+capacity_mm_ub)/2))
    NB_VEHICLES_PER_SATELLITE = Int(ceil(capacity_microhub/CAPACITY_2E))
    min_parkings_by_demand = Int(ceil(total_demand / capacity_microhub))
    println("min_parkings_by_demand = $min_parkings_by_demand")
    min_vehicles_needed = Int(ceil(total_demand / CAPACITY_2E))
    min_parkings_by_vehicles = Int(ceil(min_vehicles_needed / NB_VEHICLES_PER_SATELLITE))
    println("min_parkings_by_vehicles = $min_parkings_by_vehicles")
    minimum_parkings = max(min_parkings_by_demand, min_parkings_by_vehicles)
    println("minimum_parkings = $minimum_parkings")
    max_time = maximum(time_window_b)
    parking_avail = zeros(Int, 1 + nb_parking)
    parking_avail[2:nb_microhub+1] .= 1

    println("  Total demands : $total_demand")
    println("  Minimum parkings required: $minimum_parkings")
    println("  Minimum 2e vehicles required: $min_vehicles_needed")
    println("  Microhubs: $nb_microhub, Total parkings: $nb_parking")

    
    #==========================================================================
        SECTION 4: DEFINE NODE SETS
    ==========================================================================#
    
    n_nodes = 1 + nb_parking +nb_customer
    points_set = 1:n_nodes
    satellites_set = 2:(1 + nb_parking)                       # Parking/satellite nodes
    customers_set = (2 + nb_parking):n_nodes                  # Customer nodes
    A2_set = 2:n_nodes                                              # All nodes except depot
    A1_set = 1:(1 + length(satellites_set))                         # Depot + satellites
    
    
    #==========================================================================
        SECTION 5: SET GLOBAL VARIABLES
    ==========================================================================#
    
    # Network structure
    global nb_customer = nb_customer
    global nb_parking = nb_parking
    global nb_microhub = nb_microhub
    global coor = coor_full
    global arc_cost = arc_cost_matrix
    global demands = demands_data
    global parking_availability = parking_avail
    ...
    # x_text = maximum(coor_x) + 5
    # y_top  = maximum(coor_y)
    # dy     = 1.5
    ...
    #         y_shift = 50*1.5
    #     end
    #     annotate!(plt, coor_x[i], coor_y[i] + 0.7,
    #             text(node_labels[i-1-nb_parking], :center, 6))

    #     # y_text = y_top - dy*(k-1)

    #     # annotate!(plt, x_text, y_text - y_shift,
    #     #         Plots.text("cust[$i]: d=$(demands[i]), tw=[$(time_window_a[i]),$(time_window_b[i])]",
    #     #                     :left, 6, color=:black))
    # end
    # annotate!(plt, x_text, y_top, Plots.text("Total demands: $(total_demand)",:left, 8, color=:black))
    # annotate!(plt, x_text, y_top-1.5, Plots.text("Capacity of MM: $(capacity_microhub), Capacity of 2e vehicle: $(capacity_2e_vehicle)", :left, 8, color=:black))
    # annotate!(plt, x_text, y_top-3, Plots.text("Minimum 2e vehicles required: $(minimum_2e_vehicle_required)", :left, 8, color=:black))
    # annotate!(plt, x_text, y_top-4.5, Plots.text("Minimum microhubs required: $(minimum_parkings)", :left, 8, color=:black))
    # annotate!(plt, x_text, y_top-6, Plots.text("Number of 2e vehicle per microhub: $(nb_vehicle_per_satellite)", :left, 8, color=:black))
    # annotate!(plt, x_text, y_top-7.5, Plots.text("Number of microhubs: $nb_microhub, Number of parkings: $nb_parking", :left, 8, color=:black))

    # result_path_png = "../../Data/Instances/FigData/png/$(instance_name).png"
    # result_path_svg = "../../Data/Instances/FigData/svg/$(instance_name).svg"
