function solveBaseline()
    ...
    @constraint(model, [i in active_A2, s in active_satellites], sum(z[j,i,s] for j in active_A2 if i!= j) == sum(z[i,j,s] for j in active_A2 if i!= j))
