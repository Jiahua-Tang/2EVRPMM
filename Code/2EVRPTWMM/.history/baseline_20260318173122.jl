function solveBaseline()
    ...
    @constraint(model, [i in active_satellites, j in active_satellites], u[i] - u[j] + length(active_satellites)*x[i,j] <= length(active_satellites)-1)
    ...
    @constraint(model, [s in active_satellites], w[s] <= sum(x[j,s] for j in A1)*capacity_1e_vehicle)
    # @constraint(model, )
