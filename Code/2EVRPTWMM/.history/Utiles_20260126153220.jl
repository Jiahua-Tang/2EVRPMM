...
function read_nico_dataset(filename)
    ...
    min_parkings_by_vehicles = Int(ceil(min_vehicles_needed / NB_VEHICLES_PER_SATELLITE))
    minimum_parkings = max(min_parkings_by_demand, min_parkings_by_vehicles)
    nb_microhub = min(Int(ceil(minimum_parkings * 1.5)), nb_parking)
    max_time = maximum(time_window_b)
    ...
    println("  Microhubs: $minimum_parkings, Total parkings: $nb_parking")
    ...
    annotate!(plt, x_text, y_top-3, Plots.text("Number of 2e vehicle per microhub: $(nb_vehicle_per_satellite)", :left, 8, color=:black))
    annotate!(plt, x_text, y_top-4.5, Plots.text("Number of microhubs: $minimum_parkings, Number of total parkings: $nb_parking", :left, 8, color=:black))
