function solveBaseline()
    ...
    @variable(model, f[A2,A2,active_satellites]>=0)
    ...
    @constraint(model, [s in active_satellites], w[s] == sum(f[s, j, s] for j in A2))

    @constraint(model, [i in A2, s in active_satellites], sum(z[j,i,s] for j in A1 if i!= j) == sum(z[i,j,s] for j in A1 if i!= j))

    @constraint(model, [s in active_satellites], sum(y[s,j,s] for j in A2)<=nb_vehicle_per_satellite)

    @constraint(model, [i in active_satellites], sum(x[i,j] for j in A1 if i != j) <= 1)

    @constraint(model, [j in customers], sum(y[i,j,s] for i in A2, s in active_satellites)==1)
