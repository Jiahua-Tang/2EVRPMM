...
function read_nico_dataset(filename)
    ...
    annotate!(plt, x_text, y_top, Plots.text("Total demands: $(total_demand), Capacity of 2e vehicle: $(capacity_2e_vehicle)", :left, 8, color=:black))
    annotate!(plt, x_text, y_top-1.5, Plots.text("Minimum 2e vehicles required: $(minimum_2e_vehicle_required)", :left, 8, color=:black))
    annotate!(plt, x_text, y_top-3, Plots.text("Minimum microhubs required: $(minimum_parkings)", :left, 8, color=:black))
    annotate!(plt, x_text, y_top-4.5, Plots.text("Number of 2e vehicle per microhub: $(nb_vehicle_per_satellite)", :left, 8, color=:black))
    annotate!(plt, x_text, y_top-6, Plots.text("Number of microhubs: $nb_microhub, Number of parkings: $nb_parking", :left, 8, color=:black))
