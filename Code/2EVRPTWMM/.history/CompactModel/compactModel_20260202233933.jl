...
function displayResult(model, x, y, t, w, z, f, execution_time_limit,tau)
    ...
    if termination_status(model) == MOI.OPTIMAL
        ...
        row_data = [currentTime, "c", "\"$instance_name\"", MOI.get(model, MOI.SolveTimeSec()), objective_value(model), "/"]
    ...
    elseif primal_status(model) == MOI.FEASIBLE_POINT
        row_data = [currentTime, "c", instance_name, MOI.get(model, MOI.SolveTimeSec()), objective_value(model),MOI.get(model, MOI.RelativeGap())]
        open("./result.csv", "a") do file
            println(file, join(row_data, ",")) 
        end
        ...
        row_data = [currentTime, "c", instance_name, MOI.get(model, MOI.SolveTimeSec()), objective_value(model),"no feasible solution"]
        open("./result.csv", "a") do file
            println(file, join(row_data, ",")) 
        end
        return
