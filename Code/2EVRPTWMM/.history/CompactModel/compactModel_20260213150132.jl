...
function buildModel() 
    ...
    @constraint(model, [i in customers, j in customers], t[i]+eta2*(1-z[i,j]) + arc_cost[i,j]*z[i,j] <= t[j]+10000 * (1 - z[i,j]))
    #24
    @constraint(model, [i in setdiff(customers,9)], t[i] >= time_window[i][1])
    @constraint(model, [i in setdiff(customers,9)], t[i] <= time_window[i][2])
    #25 26
...
function displayResult(model, x, y, t, w, z, f, execution_time_limit,tau)
    ...
    if termination_status(model) == MOI.OPTIMAL
        ...
        row_data = [currentTime, "c", "\"$instance_name\"", length(customers), length(satellites), sum(parking_availability), nb_vehicle_per_satellite, MOI.get(model, MOI.SolveTimeSec()), objective_value(model), "/"]
    ...
    elseif primal_status(model) == MOI.FEASIBLE_POINT
        row_data = [currentTime, "c", "\"$instance_name\"", length(customers), length(satellites), sum(parking_availability), nb_vehicle_per_satellite, MOI.get(model, MOI.SolveTimeSec()), objective_value(model),MOI.get(model, MOI.RelativeGap())]
        open("./result.csv", "a") do file
            println(file, join(row_data, ",")) 
        end
        ...
        row_data = [currentTime, "c", "\"$instance_name\"", length(customers), length(satellites), sum(parking_availability), nb_vehicle_per_satellite, MOI.get(model, MOI.SolveTimeSec()), objective_value(model),"no feasible solution"]
        open("./result.csv", "a") do file
            println(file, join(row_data, ",")) 
        end
        return
    ...
    if primal_status(model) == MOI.FEASIBLE_POINT
        ...
        time_labels = [string("t[$i]= ", round(value(t[i]),digits=3)) for i in A2] # t
        println(time_labels)
