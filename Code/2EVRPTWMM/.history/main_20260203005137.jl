...
#            # generateData(instance_size, random_seed)
            ...
            if !isnothing(optimalSolution)
                row_data = [currentTime, "bp", "\"$filename\"", length(satellites), sum(parking_availability), nb_vehicle_per_satellite, TIME_LIMIT, objective_value(model),MOI.get(model, MOI.RelativeGap())]
