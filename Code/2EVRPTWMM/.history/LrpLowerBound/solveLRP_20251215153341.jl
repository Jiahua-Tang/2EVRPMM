...
function solve_1e_tsp_labelling(selected_parkings)
    ...
    active_nodes_set = BitSet(vcat(1, collect(selected_parkings)))
    ...
    while !isempty(label_queue) && num_iter_labelling < 6
        ...
        println("iter labelling tsp $num_iter_labelling, contains labels: ")
        for (label,_) in enumerate(label_queue)
            # println(label)
            println(label.visitedSequence, "  ", label.distance)
        end
