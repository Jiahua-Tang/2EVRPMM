...
function solve_1e_tsp_labelling(selected_parkings)
    println("solve 1e problem of: ",selected_parkings)
    if length(selected_parkings) == 1
        ...
        parking_status = zeros(Int, length(A1))
        new_label = LabelTSP(1, 0, parking_status, visit_nodes, [1])
            ...
            for node in active_nodes_set
                ...
                if !isnothing(new_label)
                    ...
                    if node == 1
                        ...
                        for n in selected_parkings 
                            if new_label.parkingStatus[n] == 0
                                terminated = false
                                break
                            end
                        end
                        ...
                        if terminated
                            for n in selected_parkings
                                if new_label.parkingStatus[n] == 0 && (n in new_label.visitedSequence)
                                valide = false
                                    println("label $(new_label.visitedSequence) invalid because of node $n")
                                break
                                end
                            end
                        else
                        ...
                        # for n in satellites
                        #     if n in selected_parkings && new_label.parkingStatus[n] == 0 && (n in new_label.visitedSequence)
                        #         valide = false
        ...
        # println("length of result labels: ",length(result_labels))
        route_1e = generate1eRoute(dequeue!(result_labels).visitedSequence)
    
    end

    println("result: ",route_1e.sequence,"\n")
    return route_1e
end
...
function extend_tsp_label(label, next_node, selected_parkings)
    ...
    parking_status = deepcopy(label.parkingStatus)
    ...
    if next_node != 1
        ...
        elseif parking_availability[label.current_node] == 1
            if parking_availability[next_node] == 0
                ...
                parking_status[label.current_node] = 0 # line seems can be delete
                parking_status[next_node] = 1
            elseif parking_availability[next_node] == 1
                ...
                parking_status[label.current_node] = 1
            end 
        end
    elseif next_node == 1
    ...
    new_label = LabelTSP(current_node, 
                         ...
                         visitedNodes,
                         visitedSequence)
    println("generated new label: ",visitedSequence, "  ", parking_status)
    return new_label
end
