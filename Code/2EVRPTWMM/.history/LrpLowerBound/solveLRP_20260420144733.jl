...
function get_sorted_2e_subproblems()
    ...
    execution_time_sp = @elapsed begin
        execution_time_build = @elapsed data = build_MDVRP_model()
        println("time to build MDVRP model = ", round(execution_time_build, digits=3), " seconds")
        for num_parking in minimum_parkings_required:nb_microhub
            for parking_subset in combinations(satellites, num_parking)
                execution_time = @elapsed route_1e = solve_1e_tsp_labelling(parking_subset)
                push!(routes_1e_complete, route_1e)
                println("1e TSP labelling time = ", round(execution_time, digits=3), " seconds")
                lower_bound_subproblem = route_1e.cost
                execution_time = @elapsed solve_MDVRP_LP!(data, parking_subset)
                println("2e LP time = ", round(execution_time, digits=3), " seconds")
                lower_bound_subproblem += execution_time
                enqueue!(lrp_subproblems, route_1e, lower_bound_subproblem)
            end
        end 

    end
    println("total time to solve all subproblems = ", round(execution_time_sp, digits=3), " seconds")
