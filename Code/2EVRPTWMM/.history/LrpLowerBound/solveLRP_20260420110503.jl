...
function get_sorted_2e_subproblems()
    ...
    function solve_LRP_LP!(data, selected_parkings)
        ...
        for i in satellites
            if i in selected
                ...
                set_normalized_rhs(data.c_open_ub[i], big_M)   # or a tight upper bound
            else
    ...
    #region: create 1e LP model
    model = Model(CPLEX.Optimizer)
    set_silent(model)
    set_optimizer_attribute(model, "CPX_PARAM_THREADS", 1)
    global x_vars = Dict{Float64, VariableRef}()
    global f_vars = Dict{Float64, VariableRef}()

    @objective(model, Min, sum(arc_cost[i,j] * x_vars[i,j] for i in A2, j in A2))
    #endregion
