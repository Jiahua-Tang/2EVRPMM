...
function solve_1e_tsp_labelling(selected_parkings)
    ...
    while !isempty(label_queue) && num_iter_labelling < 6

        println("iter labelling tsp $num_iter_labelling, contains $(length(label_queue)) labels: ")
        ...
        println("\nSelected label : $(min_label.visitedSequence)")
        current_node = min_label.current_node
        push!(processedLabels[current_node], min_label)

        for node in setdiff!(active_nodes_set, current_node)
            ...
            if !isnothing(new_label)
                ...
                if node == 1
                    push!(processedLabels[1], new_label)
                else
...
function extend_tsp_label(label, next_node, selected_parkings)
    ...
    # if label.microhubStatus == 1 && parking_availability[next_node] == 0
    #     microhubStatus = 0
    # elseif label.microhubStatus == 0 && parking_availability[next_node] == 1
    #     microhubStatus = 2
    # elseif label.microhubStatus == 0 && parking_availability[next_node] == 0
    #     return nothing
    # elseif label.microhubStatus == 1 && parking_availability[next_node] == 1
    #     # microhubStatus = 3
    # elseif label.microhubStatus == 2
