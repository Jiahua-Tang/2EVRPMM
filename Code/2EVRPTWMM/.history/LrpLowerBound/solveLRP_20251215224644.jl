    ...
    while !isempty(label_queue) && num_iter_labelling < 16
        ...
        for node in active_nodes_set
            ...
            if !isnothing(new_label)
                ...
                if node == 1
                    for n in A1 
                        if n in selected_parkings && label.parkingStatus[n] == 0
                            
                        elseif !(n in selected_parkings) && label.parkingStatus[n] == 1
                            
                        end
                    end
                    for selected_parking in selected_parkings 
                        if label.parkingStatus[selected_parking]
