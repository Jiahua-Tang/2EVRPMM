...
function get_sorted_2e_subproblems()
    ...
    function solve_MDVRP_LP!(data, selected_parkings)
        ...
        for i in satellites
            if i in selected
                ...
                # for a selected parking, set the out flow to at least 1 (open) and at most nb_vehicle_per_satellite (open)
                set_normalized_rhs(data.c_open_lb[i], 1)
                set_normalized_rhs(data.c_open_ub[i], 1000)   # or a tight upper bound
            else
    ...
    for num_parking in minimum_parkings_required:nb_microhub
        for parking_subset in combinations(satellites, num_parking)
            ...
            lower_bound_subproblem += solve_MDVRP_LP!(data, parking_subset)
