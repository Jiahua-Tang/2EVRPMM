...
#            # generateData(instance_size, random_seed)
            ...
            if !isnothing(optimalSolution)
                row_data = [currentTime, "bp", "\"$instance_name\"", length(satellites), sum(parking_availability), nb_vehicle_per_satellite, MOI.get(model, MOI.SolveTimeSec()), objective_value(model),MOI.get(model, MOI.RelativeGap())]
