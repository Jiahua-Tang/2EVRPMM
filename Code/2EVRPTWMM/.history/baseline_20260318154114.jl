function solveBaseline()
    ...
    @constraint(model, [s in active_satellites], sum(z[s,j,s] for j in A2)<=nb_vehicle_per_satellite)

    @constraint(model, [i in active_satellites], sum(x[i,j] for j in A1 if i != j) <= 1)

    @constraint(model, [j in customers], sum(z[i,j,s] for i in A2, s in active_satellites)==1)
