...
function displayResult(model, x, y, t, w, z, f, execution_time_limit, tau, t_start=time(), build_time=0.0)
    ...
    for p in satellites
        ...
        for start_cust in departures
            ...
            # arrival times along the route: depot has no t[]; customers and end-satellite use value(t[·])
            arr_strs = String[]
            for n in sev_route
                if n in satellites && n == sev_route[1]
                    push!(arr_strs, "$(n)(t=0.00)")    # start parking departure
                else
                    tn = try
                        round(value(t[n]), digits=2)
                    catch
                        missing
                    end
                    push!(arr_strs, "$(n)(t=$(tn))")
                end
            end
            println("    ", join(sev_route, " -> "),
                    "  | load = ", round(sev_load, digits=2),
                    "  | cost = ", round(sev_cost, digits=2))
            println("        arrival: ", join(arr_strs, " -> "))
        end
    end
