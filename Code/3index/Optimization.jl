    ...
    if primal_status(model) == MOI.FEASIBLE_POINT
        ...
        for i in N
            ...
            if i in A2
                ...
                # Add SEV arcs length between the locations if they are traversed
                for j in A2
                    if sum(round(value(z[i, j, k])) for k in V2) == 1
                        cox = (x_coor[i] + x_coor[j]) / 2
                        coy = (y_coor[i] + y_coor[j]) / 2  # Use y_coor here
                        annotate!(cox, coy, text(round(distances[i, j], digits=2), :center, 6)) 
                    end
                end
            end
        end  
        ...
        for k in 1:length(V2)*sum(PI)
            ...
            for i in A2
                for j in A2
                    if round(value(z[i, j, k])) == 1
                        plot!([x_coor[i], x_coor[j]], [y_coor[i], y_coor[j]],line=:arrow,color = colorR)
                    end
                end
            end 
        end  
