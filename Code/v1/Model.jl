...
function totalDurationBasic(z, x)
    if x in 2:1+np
        println("It arrives a parking")
        return 0
    end
    for k in 2:1+np+nc
        if round(value(z[x,k])) == 1
            println("z[$x, $k]=1, find next point, add distance ", distances[x,k])
            return distances[x,k] + totalDurationBasic(z, k)
        end
    end
    return 0
end

# Recursive function to accumulate total travel distance for a vehicle
function totalDuration(z, x, start_satellite)
    total_distance = 0
    current_node = x
    
    while true
        next_node = find_next_node(z, current_node)  # Define this helper function to find the next connected node
        if next_node == start_satellite  # Reached back to satellite
            break
        elseif next_node == nothing
            # No further path (end of route)
            break
        else
...
# Helper function to find the next connected node based on z[i,j] values
function find_next_node(z, current_node)
    for j in 2:1+np+nc
        if round(value(z[current_node, j])) == 1
            return j
        end
    end
    return nothing
