...
function buildModel() 
    ...
    @constraint(model, [i in P], sum(y[i,j] for j in A1)<= PI[i])
    #9
    #Can't tow a MM to a parking occupied
    @constraint(model, [j in P], sum(y[i,j] for i in A1)<=1-PI[j])
    #10
    ...
    #18
    #Customer demand met
    @constraint(model, [i in C], sum(f[j,i] for j in A2)-sum(f[i,j] for j in A2) == demands[i])
    #19
